...
class ExponentialBackoffResolutionStrategyMixin (object):
    ...
    def drive_to_resolution(self):
        ...
        #     self.retransmit_task = task.LoopingCall( lambda : self.send_accept(m.proposal_id, m.proposal_value) )
        # else:
        ...
        self.retransmit_task = task.LoopingCall( lambda: self.send_preprepare(ret.seq_num, req.message) )
    ...
    def propose_update(self, new_value, timestamp, client_id):
        super(ExponentialBackoffResolutionStrategyMixin,self).propose_update(new_value, timestamp, client_id)
        ret = self.pbft.receive_request(pbft._REQUEST, new_value, timestamp, client_id)
